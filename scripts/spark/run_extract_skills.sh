...
# ============================================
# Spark Job - Extract Skills Runner
# ============================================
# Script pour lancer le job d'extraction de compétences NLP
#
# Usage: ./run_extract_skills.sh [options]
# Options:
...
show_help() {
    cat << EOF
Script de lancement du job Spark d'extraction de compétences NLP
...
DÉPENDANCES:
- spaCy doit être installé dans l'environnement Spark
- pip install spacy
...
JOB_FILE="$PROJECT_ROOT/spark/batch/extract_skills.py"
if [[ ! -f "$JOB_FILE" ]]; then
...
SPARK_SUBMIT_CMD="docker exec bigdata_spark_master \
    ...
    /opt/spark-apps/batch/extract_skills.py"
...
# Exécution du job
log_info "Lancement du job d'extraction de compétences..."
...
if eval "$SPARK_SUBMIT_CMD"; then
    ...
    log_success "Job d'extraction de compétences terminé avec succès"
    log_success "Durée d'exécution: ${DURATION}s"

    # Suggestions pour la suite
    log_info "Prochaines étapes suggérées:"
    echo "  1. Vérifier les compétences extraites dans MinIO"
    echo "  2. Lancer l'extraction de salaires: ./run_extract_salary.sh"
    echo "  3. Examiner la qualité des extractions NLP"

else
    END_TIME=$(date +%s)
    DURATION=$((END_TIME - START_TIME))

    log_error "Échec du job d'extraction de compétences après ${DURATION}s"
    log_error "Consultez les logs Spark pour diagnostiquer l'erreur"
    log_error "Vérifiez que spaCy est installé: pip install spacy && python -m spacy download en_core_web_sm"
    exit 1
fi
